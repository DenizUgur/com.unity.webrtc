...
namespace webrtc
{
    ...
    int32_t NvEncoderImpl::ProcessEncodedFrame(std::vector<uint8_t>& packet, const ::webrtc::VideoFrame& inputFrame)
    {
        ...
        m_decoder->Decode(m_encodedImage, false, 0);

        const rtc::scoped_refptr<VideoFrameBuffer> ref_image = inputFrame.video_frame_buffer();
        const rtc::scoped_refptr<VideoFrameBuffer> new_image =
            controllerBase->GetDecodedImageCallback()->GetDecodedImage().video_frame_buffer();

        double m_ssim = ::webrtc::I420SSIM(*ref_image->GetI420(), *new_image->GetI420());
